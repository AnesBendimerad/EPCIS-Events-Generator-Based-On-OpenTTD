/* $Id: cargoaction.h 26108 2013-11-25 14:30:22Z rubidium $ */
...
protected:
	...
	Station *station;
public:
	CargoDelivery(VehicleCargoList *source, uint max_move, CargoPayment *payment,Vehicle *vehicle,Station *station) :
			CargoRemoval<VehicleCargoList>(source, max_move), payment(payment),vehicle(vehicle),station(station) {}
	bool operator()(CargoPacket *cp);
};
...
protected:
	...
	Station *station;
public:
	CargoTransfer(VehicleCargoList *source, StationCargoList *destination, uint max_move,Vehicle *vehicle,Station *station) :
			CargoMovement<VehicleCargoList, StationCargoList>(source, destination, max_move),vehicle(vehicle),station(station) {}
	bool operator()(CargoPacket *cp);
};
...
protected:
	...
	Station *station;
public:
	CargoLoad(StationCargoList *source, VehicleCargoList *destination, uint max_move, TileIndex load_place,Vehicle *vehicle,Station *station) :
			CargoMovement<StationCargoList, VehicleCargoList>(source, destination, max_move), load_place(load_place),vehicle(vehicle),station(station) {}
	bool operator()(CargoPacket *cp);
};
...
public:
	CargoReservation(StationCargoList *source, VehicleCargoList *destination, uint max_move, TileIndex load_place,Vehicle *vehicle,Station *station) :
		CargoLoad(source, destination, max_move, load_place,vehicle,station) {}
	bool operator()(CargoPacket *cp);
};
...
class CargoReturn : public CargoMovement<VehicleCargoList, StationCargoList> {
	...
	Vehicle * vehicle;
public:
	CargoReturn(VehicleCargoList *source, StationCargoList *destination, uint max_move, StationID next,Station * station,Vehicle * vehicle) :
			CargoMovement<VehicleCargoList, StationCargoList>(source, destination, max_move), next(next),station(station),vehicle(vehicle) {}
	bool operator()(CargoPacket *cp);
};
...
protected:
	Vehicle *vehicleDestination;
public:
	CargoShift(VehicleCargoList *source, VehicleCargoList *destination, uint max_move,Vehicle *vehicleDestination) :
			CargoMovement<VehicleCargoList, VehicleCargoList>(source, destination, max_move),vehicleDestination(vehicleDestination) {}
	bool operator()(CargoPacket *cp);
};
