...
#include "date_func.h"
#include "EPCISTraces/trace_manager.h"
 ...
 */
static uint DeliverGoodsToIndustry(const Station *st,CargoID cargo_type, uint num_pieces, std::vector<long long> & cargosIdentifiers, IndustryID source)
{
	...
	for (uint i = 0; i < st->industries_near.Length() && num_pieces != 0; i++) {
		...
		ind->incomingCargosWaitingIdentifiers[cargo_index].insert(ind->incomingCargosWaitingIdentifiers[cargo_index].end(),cargosIdentifiers.begin(),cargosIdentifiers.begin()+amount);
		std::vector<long long> incompingPartIds(cargosIdentifiers.begin() , cargosIdentifiers.begin()+ amount);
		TraceManager& manager=TraceManager::Instance();
		manager.addEventDeliveryGoodsToIndustry(incompingPartIds,st,ind,cargo_type);
		std::vector<long long> restOfIds(cargosIdentifiers.begin() + amount, cargosIdentifiers.end());
		cargosIdentifiers=restOfIds;
 ...
 */
static Money DeliverGoods(int num_pieces,std::vector<long long> & cargosIdentifiers, CargoID cargo_type, StationID dest, TileIndex source_tile, byte days_in_transit, Company *company, SourceType src_type, SourceID src)
{
	assert(num_pieces > 0);

	Station *st = Station::Get(dest);
	/* Give the goods to the industry. */
	uint accepted = DeliverGoodsToIndustry(st, cargo_type, num_pieces,cargosIdentifiers, src_type == ST_INDUSTRY ? src : INVALID_INDUSTRY);
...
static void TriggerIndustryProduction(Industry *i)
{
	...
	if (HasBit(callback, CBM_IND_PRODUCTION_CARGO_ARRIVAL) || HasBit(callback, CBM_IND_PRODUCTION_256_TICKS)) {
		...
		for (uint cargo_index = 0; cargo_index < lengthof(i->incoming_cargo_waiting); cargo_index++) {
			std::vector<long long> allIncomingUsedIds[3];
			std::vector<long long> allNewProducedIds[2];
			uint cargo_waiting = i->incoming_cargo_waiting[cargo_index];
			if (cargo_waiting == 0) continue;
			uint16 precProduced0=i->produced_cargo_waiting[0];
			uint16 precProduced1=i->produced_cargo_waiting[1];
			...
			uint16 newProduced0=i->produced_cargo_waiting[0]-precProduced0;
			uint16 newProduced1=i->produced_cargo_waiting[1]-precProduced1;
			int cpt;
			CargoIdManager& manager=CargoIdManager::Instance();
			long long currentId;
			for (cpt=0;cpt<newProduced0;cpt++)
			{
				currentId=manager.getNextId();
				allNewProducedIds[0].push_back(currentId);
				i->producedCargosWaitingIdentifiers[0].push_back(currentId);
			}
			for (cpt=0;cpt<newProduced1;cpt++)
			{
				currentId=manager.getNextId();
				allNewProducedIds[1].push_back(currentId);
				i->producedCargosWaitingIdentifiers[1].push_back(currentId);
			}
			allIncomingUsedIds[cargo_index]=i->incomingCargosWaitingIdentifiers[cargo_index];
			TraceManager& traceManager=TraceManager::Instance();
			traceManager.addEventGoodsTransformation(allIncomingUsedIds,allNewProducedIds,i->produced_cargo,i->accepts_cargo, i);
			i->incoming_cargo_waiting[cargo_index] = 0;
			i->incomingCargosWaitingIdentifiers[cargo_index].clear();
		}
	}
 ...
 */
void CargoPayment::PayFinalDelivery(const CargoPacket *cp, uint count,std::vector<long long> & cargosIdentifiers)
{
	if (this->owner == NULL) {
	...
	Money profit = DeliverGoods(count, cargosIdentifiers,this->ct, this->current_station, cp->SourceStationXY(), cp->DaysInTransit(), this->owner, cp->SourceSubsidyType(), cp->SourceSubsidyID());
...
struct ReturnCargoAction
{
	...
	{
		v->cargo.Return(UINT_MAX, &this->st->goods[v->cargo_type].cargo, this->next_hop,st,v);
		return true;
	}
};
	...
	bool operator()(Vehicle *v)
	{
		if (this->do_reserve) {
			this->st->goods[v->cargo_type].cargo.Reserve(v->cargo_cap - v->cargo.RemainingCount(),
					&v->cargo, st->xy, this->next_station,v,st);
		}
	...
	bool operator()(Vehicle *v)
	{
		if (v->cargo_cap > v->cargo.RemainingCount()) {
			st->goods[v->cargo_type].cargo.Reserve(v->cargo_cap - v->cargo.RemainingCount(),
					&v->cargo, st->xy, *next_station,v,st);
		}
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	std::cout <<  " (" << _date << ") - "  << int(ymd.day) <<"/"<<ymd.month+1 <<"/"<<ymd.year <<  ": " <<buf;	
	...
	std::cout << buf << " is offering:" << std::endl;
	GoodsEntry *geo = &st->goods[front->cargo_type];
	for(StationCargoPacketMap::const_iterator it=geo->cargo.Packets()->begin();it!=geo->cargo.Packets()->end();++it) { 
		std::cout << "("<< (*it)->Count()<<") ";
		std::cout << "Age: " << int((*it)->DaysInTransit());
		if (BaseStation::GetIfValid((*it)->SourceStation())) {
			::SetDParam(0, (*it)->SourceStation());
			GetString(buf,STR_STATION_NAME,lastof(buf));
			std::cout << " From: " << buf;
			}
		if (BaseStation::GetIfValid((*it)->NextStation())) {
				::SetDParam(0, (*it)->NextStation());
				GetString(buf,STR_STATION_NAME,lastof(buf));
				std::cout << " To: " << buf;
			}
		std::cout << std::endl;
	...
	*/
	...
	for (Vehicle *v = front; v != NULL; v = v->Next()) {
		...
		if (HasBit(v->vehicle_flags, VF_CARGO_UNLOADING) && (front->current_order.GetUnloadType() & OUFB_NO_UNLOAD) == 0) {
			...
			if (!HasBit(ge->status, GoodsEntry::GES_ACCEPTANCE) && v->cargo.ActionCount(VehicleCargoList::MTA_DELIVER) > 0) {
				...
				if (front->current_order.GetUnloadType() & (OUFB_TRANSFER | OUFB_UNLOAD)) {
					...
					if (v->cargo_cap < new_remaining) {
						...
						v->cargo.Return(new_remaining - v->cargo_cap, &ge->cargo, INVALID_STATION,st,v);
					}
			...
			amount_unloaded = v->cargo.Unload(amount_unloaded, &ge->cargo, payment,v,st);
		...
		if (cap_left > 0 && (v->cargo.ActionCount(VehicleCargoList::MTA_LOAD) > 0 || ge->cargo.AvailableCount() > 0)) {
			...
			uint loaded = ge->cargo.Load(cap_left, &v->cargo, st->xy, next_station,v,st);
