...
#include "EPCISTraces/cargo_id_manager.h"
#include <vector>
#include <list>
...
public:
	...
	CargoPacket(std::vector<long long> &cargosIdentifiers,StationID source, TileIndex source_xy, uint16 count, SourceType source_type, SourceID source_id);
	CargoPacket(uint16 count, byte days_in_transit, StationID source, TileIndex source_xy, TileIndex loaded_at_xy, Money feeder_share = 0, SourceType source_type = ST_INDUSTRY, SourceID source_id = INVALID_SOURCE);
	CargoPacket(std::vector<long long> &cargosIdentifiers, uint16 count, byte days_in_transit, StationID source, TileIndex source_xy, TileIndex loaded_at_xy, Money feeder_share = 0, SourceType source_type = ST_INDUSTRY, SourceID source_id = INVALID_SOURCE);
	...
	void updateCargosIdentifiers();


	std::vector<long long> & getCargosIdentifiers()  { return cargosIdentifiers;}
	...
	{
		return this->feeder_share * part / static_cast<uint>(this->count2);
	}
...
public:
	...
	uint Return(uint max_move, StationCargoList *dest, StationID next_station,Station *station,Vehicle *vehicle);
	uint Unload(uint max_move, StationCargoList *dest, CargoPayment *payment,Vehicle *vehicle,Station *station);
	uint Shift(uint max_move, VehicleCargoList *dest,Vehicle *v);
...
public:
	...
	uint Reserve(uint max_move, VehicleCargoList *dest, TileIndex load_place, StationIDStack next,Vehicle *vehicle,Station *station);
	uint Load(uint max_move, VehicleCargoList *dest, TileIndex load_place, StationIDStack next,Vehicle *vehicle,Station *station);
