/* $Id: vehicle.cpp 27222 2015-04-09 18:17:43Z planetmaker $ */
...
void CallVehicleTicks()
{
	...
	FOR_ALL_VEHICLES(v) {
		...
		//if ((GB(v->First()->tick_counter,0,6) == 0) && (v->cargo.TotalCount())){
  //  			::YearMonthDay ymd;
  ...
  //          	GetString(buf,STR_VEHICLE_NAME,lastof(buf));
		//		//char trainName[]="H-J";
		//		//if (strcmp(buf,trainName)==0){
		//		std::cout << "<tr>";
		//		std::cout << "<tc>"<< int(v->tick_counter) <<"</tc>";
	 //           std::cout << "<date>" << int(ymd.day) <<"/"<<ymd.month+1 <<"/"<<ymd.year <<  "</date>";
  //  			std::cout << "<vid>" << v->index << "</vid>";
  //  			std::cout << "<name>" << buf << "</name>";
  //  			std::cout << "<br>" << (int(v->breakdown_ctr)) << "</br>";
  //  			std::cout << "<ct>" << (int(v->cargo_type)) << "</ct>";
  //  			
  //              bool no_dest = true;    
  //              if (v->First()->current_order.IsType(OT_GOTO_STATION)  || v->First()->current_order.IsType(OT_GOTO_WAYPOINT) || v->First()->current_order.IsType(OT_LOADING)) {
  //                  ::SetDParam(0, v->First()->current_order.GetDestination());
  //              	GetString(buf,STR_STATION_NAME,lastof(buf));
  //                  std::cout << "<ns>" << buf << "</ns>";
  //                  no_dest = false;
  //              }
  //              std::cout<<"<order>" << int(v->First()->current_order.GetType()) << "</order>";
  //  			
  //  			std::cout << "<pkgs>";
		//		
  ...
  //          	std::cout << "<pkg>";		    
		//        if (!no_dest) {
  //                  Money profit = GetTransportedGoodsIncome((*it)->Count(),DistanceManhattan((*it)->LoadedAtXY(), Station::Get(v->First()->current_order.GetDestination())->xy),v->First()->current_order.GetTravelTime()/74,v->cargo_type);
  //                  profit = profit * _settings_game.economy.feeder_payment_share / 100;
  //                  std::cout << "<ncash>" << profit <<"</ncash>";
  //              }
		//		assert((*it)->getCargosIdentifiers().size()==(*it)->Count());
	 //   
...
void Vehicle::CancelReservation(StationID next, Station *st)
{
	for (Vehicle *v = this; v != NULL; v = v->next) {
		...
		if (cargo.ActionCount(VehicleCargoList::MTA_LOAD) > 0) {
			...
			cargo.Return(UINT_MAX, &st->goods[v->cargo_type].cargo, next,st,v);
