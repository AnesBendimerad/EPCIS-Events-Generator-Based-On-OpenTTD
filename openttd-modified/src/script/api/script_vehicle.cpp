...
/* static */ int32 ScriptVehicle::GetSource(VehicleID vehicle_id)
{
	if (!IsValidVehicle(vehicle_id)) return -1;
	//if (!ScriptCargo::IsValidCargo(cargo)) return -1;
	...
	if (GetNumWagons(vehicle_id) == 1) {
		//std::cout << "Packet list size for vehicle " << vehicle_id << ":" << v->cargo.Packets()->size() << std::endl;
		int i = 1;
		/*for(CargoPacketList::const_iterator it=v->cargo.Packets()->begin();it!=v->cargo.Packets()->end();++it) { 
			std::cout << "<tr>";
			std::cout << "<vid>" << vehicle_id << "</vid>";
			std::cout << "<ci>" << i++ << "</ci>";
			std::cout << "<n>" << (*it)->Count() <<"</n>";
			if (::ScriptStation::IsValidStation((*it)->SourceStation()))
				std::cout << "<ss>" << ::ScriptStation::GetName((*it)->SourceStation()) << "</ss>";
			SourceType st = (*it)->SourceSubsidyType();
			if (st == ST_TOWN)
				std::cout << "<st>" << ::ScriptTown::GetName((*it)->SourceSubsidyID()) << "</st>";
			std::cout << "<dit>" << (int((*it)->DaysInTransit())) << "</dit>";
			ScriptDate::Date date = ::ScriptDate::GetCurrentDate();
			if (::ScriptDate::IsValidDate(date)) {
				std::cout << "<date>";
				// GET IT IN ISO-8601
				std::cout <<::ScriptDate::GetYear(date) << "-";
				if (::ScriptDate::GetMonth(date) < 10)
					std::cout<<"0";
				std::cout <<::ScriptDate::GetMonth(date) << "-";
				if (::ScriptDate::GetDayOfMonth(date) < 10)
					std::cout << "0";
				std::cout <<::ScriptDate::GetDayOfMonth(date);
				std::cout << "</date>";
			}
			if (::ScriptStation::IsValidStation((*it)->NextStation()))
				std::cout << "<fs>" << ::ScriptStation::GetName((*it)->NextStation()) << "</fs>";
			
			if (::ScriptStation::IsValidStation(::ScriptStation::GetStationID(::ScriptOrder::GetOrderDestination(vehicle_id,::ScriptOrder::ORDER_CURRENT))))
				std::cout << "<nxs>" << ::ScriptStation::GetName(::ScriptStation::GetStationID(::ScriptOrder::GetOrderDestination(vehicle_id,::ScriptOrder::ORDER_CURRENT))) << "</nxs>";
			
			if (::ScriptStation::IsValidStation(::ScriptStation::GetStationID((*it)->LoadedAtXY())))
				std::cout << "<ls>" << ::ScriptStation::GetName(::ScriptStation::GetStationID((*it)->LoadedAtXY())) << "</ls>";
			
			
				
			std::cout << "</tr>" << std::endl;*/
			/*			
			std::cout << std::endl << "\t-> CargoPacket " << i++ << std::endl;
			std::cout << "\t-> Content: " << (*it)->Count() << std::endl;
			if (::ScriptStation::IsValidStation((*it)->SourceStation()))
				std::cout << "\t-> Source Gare: " << ::ScriptStation::GetName((*it)->SourceStation())<< std::endl;
			SourceType st = (*it)->SourceSubsidyType();
			if (st == ST_TOWN)
				std::cout << "\t-> Source Place: " << ::ScriptTown::GetName((*it)->SourceSubsidyID())<< std::endl;
			std::cout << "\t-> Days in transit: " << int((*it)->DaysInTransit()) << std::endl;
			if (::ScriptStation::IsValidStation((*it)->NextStation()))
				std::cout << "\t-> Next st.: " << ::ScriptStation::GetName((*it)->NextStation()) << std::endl;

			
		}*/
		
		return v->cargo.Source();
	}
	else
		return 404;
}
