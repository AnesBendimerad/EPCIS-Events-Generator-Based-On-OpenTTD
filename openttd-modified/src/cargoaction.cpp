 ...
 *         it was.Prondingville is offering:
...
bool CargoDelivery::operator()(CargoPacket *cp)
{
	...
	std::vector<long long> secondPart(cp->getCargosIdentifiers().begin() + cp->getCargosIdentifiers().size()-remove, cp->getCargosIdentifiers().end());
	TraceManager& manager=TraceManager::Instance();
	manager.addEventPacketToStation(cp,secondPart,this->station,vehicle);
	this->payment->PayFinalDelivery(cp, remove,secondPart);
	return this->Postprocess(cp, remove);
}
...
bool CargoLoad::operator()(CargoPacket *cp)
{
	...
	//char buf[64];
	//::SetDParam(0, BaseStation::Get(cp->SourceStation())->index);
	//GetString(buf,STR_STATION_NAME,lastof(buf));
	//std::cout << "Requested Load!!! " << cp->Count() << " : " << buf	 << " - Days in Transit:" << int(cp->DaysInTransit()) << std::endl;	
	//if (BaseStation::GetIfValid(cp->NextStation())) {
	//	::SetDParam(0, BaseStation::Get(cp->NextStation())->index);
	//	GetString(buf,STR_STATION_NAME,lastof(buf));
	//	std::cout << "\t-> Next Station:" << buf << std::endl;
	//	}
	//else
	//	std::cout << "\t-> Invalid Next Station:" << std::endl;
	...
	TraceManager& manager=TraceManager::Instance();
	manager.addEventPacketToVehicle(cp_new,cp_new->getCargosIdentifiers(),this->vehicle,this->station);
			
	this->destination->Append(cp_new, VehicleCargoList::MTA_KEEP);
	
	//::SetDParam(0, BaseStation::Get(cp_new->SourceStation())->index);
	//GetString(buf,STR_STATION_NAME,lastof(buf));
	//std::cout << "Accepted Load!!! " << cp_new->Count() << " : " << buf	 << " - Days in Transit:" << int(cp_new->DaysInTransit()) << std::endl;	
	//if (BaseStation::GetIfValid(cp_new->NextStation())) {
	//	::SetDParam(0, BaseStation::Get(cp_new->NextStation())->index);
	//	GetString(buf,STR_STATION_NAME,lastof(buf));
	//	std::cout << "\t-> Next Station:" << buf << std::endl;
	//	}
	//else
...
bool CargoReturn::operator()(CargoPacket *cp)
{
	...
	TraceManager& manager=TraceManager::Instance();
	manager.addEventPacketToStation(cp_new,cp_new->getCargosIdentifiers(),this->station,vehicle);
...
/*	char buf[64];
	::SetDParam(0, BaseStation::Get(cp->SourceStation())->index);
	GetString(buf,STR_STATION_NAME,lastof(buf));
	std::cout << "Requested Transfer!!! " << cp->Count() << " : " << buf	 << " - " << int(cp->DaysInTransit()) << std::endl;	
	if (BaseStation::GetIfValid(cp->NextStation())) {
		::SetDParam(0, BaseStation::Get(cp->NextStation())->index);
		GetString(buf,STR_STATION_NAME,lastof(buf));
		std::cout << "\t-> Next Station:" << buf << std::endl;
...
*/
	...
	//GetString(buf,STR_STATION_NAME,lastof(buf));
	//Event currentEvent(onStation,buf,ymd);
	//::SetDParam(0, cp_new->SourceStation());
    //GetString(buf,STR_STATION_NAME,lastof(buf));
	...
	::SetDParam(0, BaseStation::Get(cp_new->SourceStation())->index);
	GetString(buf,STR_STATION_NAME,lastof(buf));
	std::cout << "Accepted Transfer!!! " << cp_new->Count() << " : " << buf	 << " - " << int(cp_new->DaysInTransit()) << std::endl;	
	if (BaseStation::GetIfValid(cp_new->NextStation())) {
		::SetDParam(0, BaseStation::Get(cp_new->NextStation())->index);
		GetString(buf,STR_STATION_NAME,lastof(buf));
		std::cout << "\t-> Next Station:" << buf << std::endl;
...
bool CargoShift::operator()(CargoPacket *cp)
{
	...
	//TraceManager& manager=TraceManager::Instance();
	//manager.addEventPacketToVehicle(cp_new,cp_new->getCargosIdentifiers(),this->vehicleDestination);
