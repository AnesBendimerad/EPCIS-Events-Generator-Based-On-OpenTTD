/* $Id: station_cmd.cpp 27256 2015-04-26 11:43:58Z frosch $ */
...
static CommandCost BuildStationPart(Station **st, DoCommandFlag flags, bool reuse, TileArea area, StationNaming name_class)
{
	...
	//manager.addStationToMasterDataFile(*st);
	return CommandCost();
}
...
void OnTick_Station()
{
	...
	FOR_ALL_STATIONS(station){
		...
		//std::cout << "<sttr>";
		//std::cout << "<date>" << int(ymd.day) <<"/"<<ymd.month+1 <<"/"<<ymd.year <<  "</date>";
		//std::cout << "<sid>" << station->index << "</sid>";
		//std::cout << "<name>" << buf << "</name>";
		
		//for (int i=0;i<NUM_CARGO;i++){
		//	for(StationCargoPacketMap::const_iterator it=station->goods[i].cargo.Packets()->begin();it!=station->goods[i].cargo.Packets()->end();++it) {
				//std::cout << "<pkg>";
				//std::cout << "<cash>" << (*it)->FeederShare() <<"</cash>";
    			//std::cout << "<n>" << (*it)->Count() <<"</n>";
    			//std::cout << "<dit>" << (int((*it)->DaysInTransit())) << "</dit>";
        	    //::SetDParam(0, (*it)->SourceStation());
            	//GetString(buf,STR_STATION_NAME,lastof(buf));
                //std::cout << "<ss>" << buf << "</ss>";
                //::SetDParam(0, ::GetStationIndex((*it)->LoadedAtXY()));
            	//GetString(buf,STR_STATION_NAME,lastof(buf));
                //std::cout << "<ls>" << buf << "</ls>";               

				//std::cout<<"<cargoIds>";
				//bool firstOne=true;
				//if ((*it)->getCargosIdentifiers().size()!=(*it)->Count()){
				//	(*it)->updateCargosIdentifiers();
				//}
				//for (int i = 0; i < (*it)->getCargosIdentifiers().size(); i++) {
				//	if (firstOne)
				//	{
				//		firstOne=false;
				//	}
				...
				//		std::cout<<",";
				//	}
				//	std::cout<<(*it)->getCargosIdentifiers()[i];
				}
				//std::cout<<"</cargoIds>";
...
static uint UpdateStationWaiting(Station *st, CargoID type, uint amount,std::vector<long long> & cargoIds,TileIndex tile, SourceType source_type, SourceID source_id)
{
	...
	uint floorAmount=amount;
	floorAmount>>=8;
	std::vector<long long> sentCargoIds(cargoIds.begin(),  cargoIds.begin()+floorAmount);
	...
	if (amount==floorAmount){
		std::vector<long long> restIds(cargoIds.begin()+floorAmount,  cargoIds.end());
		cargoIds=restIds;
	}
	else {
		if (ge.fractalCargoId!=-1){
			std::vector<long long> restIds(cargoIds.begin()+floorAmount,  cargoIds.end());
			cargoIds=restIds;
			sentCargoIds.push_back(ge.fractalCargoId);
			ge.fractalCargoId=-1;
		}
		else if (cargoIds.size()>=amount){
			sentCargoIds.push_back(cargoIds[amount-1]);
			std::vector<long long> restIds(cargoIds.begin()+amount,  cargoIds.end());
			cargoIds=restIds;			
		}
		else {
			CargoIdManager& manager=CargoIdManager::Instance();
			sentCargoIds.push_back(manager.getNextId());
		}
	}
	...
	traceManager.addEventTransportIndustryGoods(sentCargoIds,tile,st,type);

	StationID next = ge.GetVia(st->index);
	CargoPacket *packet= new CargoPacket(sentCargoIds,st->index, st->xy, amount, source_type, source_id);

	ge.cargo.Append(packet, next);
	LinkGraph *lg = NULL;
...
uint MoveGoodsToStation(CargoID type, uint amount, std::vector<long long> & cargoIds, TileIndex tile,SourceType source_type, SourceID source_id, const StationList *all_stations)
{
	...
	if (st2 == NULL) {
		...
		uint returnedValue=UpdateStationWaiting(st1, type, amount, cargoIds,tile,source_type, source_id);
		if (st1->goods[type].fractalCargoId==-1 && cargoIds.size()>0){
			st1->goods[type].fractalCargoId=cargoIds[0];
			cargoIds.erase(cargoIds.begin());
		}
	...
	uint moved = UpdateStationWaiting(st1, type, amount - worst_cargo,cargoIds, tile, source_type, source_id);
	...
	uint returnedValue=moved + UpdateStationWaiting(st2, type, worst_cargo, cargoIds,tile, source_type, source_id);
	if (st1->goods[type].fractalCargoId==-1 && cargoIds.size()>0){
			st1->goods[type].fractalCargoId=cargoIds[0];
			cargoIds.erase(cargoIds.begin());
	}
	if (st2->goods[type].fractalCargoId==-1 && cargoIds.size()>0){
			st2->goods[type].fractalCargoId=cargoIds[0];
			cargoIds.erase(cargoIds.begin());
	}
	return returnedValue;
}
