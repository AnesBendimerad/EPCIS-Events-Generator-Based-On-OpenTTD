/* $Id: town_cmd.cpp 27260 2015-04-28 17:25:21Z frosch $ */
...
static void TileLoop_Town(TileIndex tile)
{
	...
	if (HasBit(hs->callback_mask, CBM_HOUSE_PRODUCE_CARGO)) {
		for (uint i = 0; i < 256; i++) {
			...
			std::vector<long long> cargoIds;
			uint cpt;
			CargoIdManager& manager=CargoIdManager::Instance();
			for (cpt=0;cpt<amt;cpt++)
			{
				cargoIds.push_back(manager.getNextId());
			}
			TraceManager & traceManager=TraceManager::Instance();
			traceManager.addEventGoodsCreation(cargoIds,t->xy,cargo,false);

			uint moved = MoveGoodsToStation(cargo, amt,cargoIds,t->xy, ST_TOWN, t->index, stations.GetStations());
		...
		if (GB(r, 0, 8) < hs->population) {
			...
			TraceManager & traceManager=TraceManager::Instance();
			traceManager.addEventGoodsCreation(cargoIds,t->xy,CT_PASSENGERS,false);

			t->supplied[CT_PASSENGERS].new_act += MoveGoodsToStation(CT_PASSENGERS, amt,cargoIds,t->xy, ST_TOWN, t->index, stations.GetStations());
		}
		...
		if (GB(r, 8, 8) < hs->mail_generation) {
			...
			TraceManager & traceManager=TraceManager::Instance();
			traceManager.addEventGoodsCreation(cargoIds,t->xy,CT_MAIL,false);

			t->supplied[CT_MAIL].new_act += MoveGoodsToStation(CT_MAIL, amt, cargoIds,t->xy, ST_TOWN, t->index, stations.GetStations());
		}
	}
