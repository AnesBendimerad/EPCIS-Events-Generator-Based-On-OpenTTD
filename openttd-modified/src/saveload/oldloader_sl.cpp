...
static bool LoadOldGood(LoadgameState *ls, int num)
{
	...
	if (GB(_waiting_acceptance, 0, 12) != 0 && CargoPacket::CanAllocateItem()) {
		CargoPacket *cp=new CargoPacket(GB(_waiting_acceptance, 0, 12), _cargo_days, (_cargo_source == 0xFF) ? INVALID_STATION : _cargo_source, 0, 0);
		//TraceManager& traceManager=TraceManager::Instance();
		//traceManager.addEventPacketToStation(cp,cp->getCargosIdentifiers(),st);
		ge->cargo.Append(cp,
				INVALID_STATION);
	}
...
static bool LoadOldIndustry(LoadgameState *ls, int num)
{
	...
	for (cpt=0;cpt<i->incoming_cargo_waiting[0];cpt++)
	{
		i->incomingCargosWaitingIdentifiers[0].push_back(manager.getNextId());
	}
	for (cpt=0;cpt<i->incoming_cargo_waiting[1];cpt++)
	{
		i->incomingCargosWaitingIdentifiers[1].push_back(manager.getNextId());
	}
	for (cpt=0;cpt<i->incoming_cargo_waiting[2];cpt++)
	{
		i->incomingCargosWaitingIdentifiers[2].push_back(manager.getNextId());
	}
	for (cpt=0;cpt<i->produced_cargo_waiting[0];cpt++)
	{
		i->producedCargosWaitingIdentifiers[0].push_back(manager.getNextId());
	}
	for (cpt=0;cpt<i->produced_cargo_waiting[1];cpt++)
	{
		i->producedCargosWaitingIdentifiers[1].push_back(manager.getNextId());
	}
...
bool LoadOldVehicle(LoadgameState *ls, int num)
{
	...
	for (uint i = 0; i < _old_vehicle_multiplier; i++) {
		...
		if (_cargo_count != 0 && CargoPacket::CanAllocateItem()) {			
			...
			CargoPacket *cp=new CargoPacket(_cargo_count, _cargo_days, source, source_xy, source_xy);
			//TraceManager& manager=TraceManager::Instance();
			//manager.addEventPacketToVehicle(cp,cp->getCargosIdentifiers(),v);
