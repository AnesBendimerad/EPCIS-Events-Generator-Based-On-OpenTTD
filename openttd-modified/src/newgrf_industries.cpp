/* $Id: newgrf_industries.cpp 27279 2015-05-09 10:21:55Z frosch $ */
...
void IndustryProductionCallback(Industry *ind, int reason)
{
	...
	for (uint loop = 0;; loop++) {
		...
		for (uint i = 0; i < 3; i++) {
			precAmount=ind->incoming_cargo_waiting[i];
			...
			diffAmount=precAmount-ind->incoming_cargo_waiting[i];
			std::vector<long long> usedIncoming(ind->incomingCargosWaitingIdentifiers[i].begin(),  ind->incomingCargosWaitingIdentifiers[i].begin()+diffAmount);
			std::vector<long long> restOfIncoming(ind->incomingCargosWaitingIdentifiers[i].begin()+diffAmount,  ind->incomingCargosWaitingIdentifiers[i].end());
			ind->incomingCargosWaitingIdentifiers[i]=restOfIncoming;
			allIncomingUsedIds[i].insert(allIncomingUsedIds[i].end(),usedIncoming.begin(),usedIncoming.end());
		}
		long long newId=0;
		for (uint i = 0; i < 2; i++) {
			precAmount=ind->produced_cargo_waiting[i];
			...
			diffAmount=ind->produced_cargo_waiting[i]-precAmount;
			CargoIdManager& manager=CargoIdManager::Instance();
			for (cpt=0;cpt<diffAmount;cpt++)
			{
				newId=manager.getNextId();
				allNewProducedIds[i].push_back(newId);
				ind->producedCargosWaitingIdentifiers[i].push_back(newId);
			}
		}
		TraceManager& traceManager=TraceManager::Instance();
		traceManager.addEventGoodsTransformation(allIncomingUsedIds,allNewProducedIds,ind->produced_cargo,ind->accepts_cargo,ind);
