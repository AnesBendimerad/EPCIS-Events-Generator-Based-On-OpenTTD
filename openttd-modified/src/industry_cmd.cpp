...
static void TransportIndustryGoods(TileIndex tile)
{
	...
	for (uint j = 0; j < lengthof(i->produced_cargo_waiting); j++) {
		...
		if (cw > indspec->minimal_cargo && i->produced_cargo[j] != CT_INVALID) {
			...
			std::vector<long long> producedToSend(i->producedCargosWaitingIdentifiers[j].begin(),  i->producedCargosWaitingIdentifiers[j].begin()+cw);
			std::vector<long long> restOfProduced(i->producedCargosWaitingIdentifiers[j].begin()+cw,  i->producedCargosWaitingIdentifiers[j].end());
			i->producedCargosWaitingIdentifiers[j]=restOfProduced;
			...
			uint am = MoveGoodsToStation(i->produced_cargo[j], cw,producedToSend, i->location.tile,ST_INDUSTRY, i->index, stations.GetStations());
...
static void ChopLumberMillTrees(Industry *i)
{
	...
	if (CircularTileSearch(&tile, 40, SearchLumberMillTrees, NULL)) { // 40x40 tiles  to search.
		uint16 precProducedValue=i->produced_cargo_waiting[0] ;
		i->produced_cargo_waiting[0] = min(0xffff, i->produced_cargo_waiting[0] + 45); // Found a tree, add according value to waiting cargo.
		uint16 newAddedValue=i->produced_cargo_waiting[0]-precProducedValue;
		CargoIdManager& manager=CargoIdManager::Instance();
		int cpt;
		std::vector<long long> newProducedIds;
		long long newId;
		for (cpt=0;cpt<newAddedValue;cpt++)
		{
			newId=manager.getNextId();
			newProducedIds.push_back(newId);
			i->producedCargosWaitingIdentifiers[0].push_back(newId);
		}
		TraceManager & traceManager=TraceManager::Instance();
		traceManager.addEventGoodsCreation(newProducedIds,tile,i->produced_cargo[0],true);
	}
}
...
static void ProduceIndustryGoods(Industry *i)
{
	...
	if ((i->counter % INDUSTRY_PRODUCE_TICKS) == 0) {
		...
		int cpt;
		long long newId;
		std::vector<long long> newProducedIds0;
		for (cpt=0;cpt<newAddedValue;cpt++)
		{
			newId=manager.getNextId();
			newProducedIds0.push_back(newId);
			i->producedCargosWaitingIdentifiers[0].push_back(newId);
		}
		TraceManager & traceManager=TraceManager::Instance();
		traceManager.addEventGoodsCreation(newProducedIds0,i->location.tile,i->produced_cargo[0],true);

		precProducedValue=i->produced_cargo_waiting[1] ;
		i->produced_cargo_waiting[1] = min(0xffff, i->produced_cargo_waiting[1] + i->production_rate[1]);
		newAddedValue=i->produced_cargo_waiting[1]-precProducedValue;
		std::vector<long long> newProducedIds1;
		for (cpt=0;cpt<newAddedValue;cpt++)
		{
			newId=manager.getNextId();
			newProducedIds1.push_back(newId);
			i->producedCargosWaitingIdentifiers[1].push_back(newId);
		}
		traceManager.addEventGoodsCreation(newProducedIds1,i->location.tile,i->produced_cargo[1],true);
	...
	*ip = NULL;
	...
	if (flags & DC_EXEC) {
		*ip = new Industry(tile);
		...
		manager.addIndustryToMasterDataFile(*ip);
	}
