...
static void TileLoop_Object(TileIndex tile)
{
	...
	if (GB(r, 0, 8) < (256 / 4 / (6 - level))) {
		Town *town=ClosestTownFromTile(tile,5000000);
		TileIndex locationTile=tile;
		if (town!=NULL)
		{
			tile=town->xy;
		}
		...
		traceManager.addEventGoodsCreation(cargoIds,locationTile,CT_PASSENGERS,false);
		MoveGoodsToStation(CT_PASSENGERS, amt,cargoIds,locationTile, ST_HEADQUARTERS, GetTileOwner(tile), stations.GetStations());
	}
	...
	if (GB(r, 8, 8) < (196 / 4 / (6 - level))) {
		...
		TraceManager & traceManager=TraceManager::Instance();
		traceManager.addEventGoodsCreation(cargoIds,locationTile,CT_MAIL,false);

		MoveGoodsToStation(CT_MAIL, amt,cargoIds, locationTile, ST_HEADQUARTERS, GetTileOwner(tile), stations.GetStations());
	}
}
