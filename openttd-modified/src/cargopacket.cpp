 ...
 */
CargoPacket::CargoPacket(std::vector<long long> &cargosIdentifiers,StationID source, TileIndex source_xy, uint16 count, SourceType source_type, SourceID source_id) :
	cargosIdentifiers(cargosIdentifiers),
	feeder_share(0),
	count2(count),
	days_in_transit(0),
	source_id(source_id),
		...
		loaded_at_xy(loaded_at_xy)
{
	assert(count != 0);
	this->source_type = source_type;
	CargoIdManager& manager=CargoIdManager::Instance();
	for (int i=0;i<count;i++)
	{
		cargosIdentifiers.push_back(manager.getNextId());
	}

}
CargoPacket:: ~CargoPacket(){
	//TraceManager& manager=TraceManager::Instance();
	//manager.closeAndSaveTraces(cargosIdentifiers);
}

CargoPacket::CargoPacket(std::vector<long long> &cargosIdentifiers,uint16 count, byte days_in_transit, StationID source, TileIndex source_xy, TileIndex loaded_at_xy, Money feeder_share, SourceType source_type, SourceID source_id) :
		feeder_share(feeder_share),
		count2(count),
		days_in_transit(days_in_transit),
		source_id(source_id),
...
{
	assert(count != 0);
	assert(count==cargosIdentifiers.size());
	this->source_type = source_type;
	CargoPacket::cargosIdentifiers=cargosIdentifiers;
}
...
CargoPacket *CargoPacket::Split(uint new_size)
{
	...
	assert(count2==cargosIdentifiers.size());
	Money fs = this->FeederShare(new_size);
	std::vector<long long> firstPart(cargosIdentifiers.begin(), cargosIdentifiers.begin() + new_size);
	std::vector<long long> secondPart(cargosIdentifiers.begin()+new_size, cargosIdentifiers.end());
	cargosIdentifiers=secondPart;

	CargoPacket *cp_new = new CargoPacket(firstPart,new_size, this->days_in_transit, this->source, this->source_xy, this->loaded_at_xy, fs, this->source_type, this->source_id);
	this->feeder_share -= fs;
	this->count2 -= new_size;
	assert(count2==cargosIdentifiers.size());
	return cp_new;
}
...
{
	assert(count2==cargosIdentifiers.size());
	assert(cp->Count()==cp->getCargosIdentifiers().size());
	//updateCargosIdentifiers();
	//cp->updateCargosIdentifiers();
	this->count2 += cp->count2;
	this->feeder_share += cp->feeder_share;
	std::vector<long long> mergedCargosIds;
	mergedCargosIds.reserve(cargosIdentifiers.size()+cp->getCargosIdentifiers().size());
	mergedCargosIds.insert( mergedCargosIds.end(), cp->getCargosIdentifiers().begin(), cp->getCargosIdentifiers().end() );
	mergedCargosIds.insert( mergedCargosIds.end(), cargosIdentifiers.begin(), cargosIdentifiers.end() );
	cargosIdentifiers=mergedCargosIds;
	assert(count2==cargosIdentifiers.size());
	cp->getCargosIdentifiers().clear();
	delete cp;
}
...
{
	assert(count < this->count2);
	assert(count2==cargosIdentifiers.size());
	this->feeder_share -= this->FeederShare(count);
	this->count2 -= count;
	std::vector<long long> firstPart(cargosIdentifiers.begin(), cargosIdentifiers.begin() + cargosIdentifiers.size()-count);
	std::vector<long long> secondPart(cargosIdentifiers.begin() + cargosIdentifiers.size()-count, cargosIdentifiers.end());
	//TraceManager& manager=TraceManager::Instance();
	//manager.closeAndSaveTraces(secondPart);
	cargosIdentifiers=firstPart;

	assert(count2==cargosIdentifiers.size());
	
}
void  CargoPacket::updateCargosIdentifiers(){
	assert((cargosIdentifiers.size()==count2)|| (cargosIdentifiers.size()==0));
	if (cargosIdentifiers.size()==0)
	{
		CargoIdManager& manager=CargoIdManager::Instance();
		for (int i=cargosIdentifiers.size();i<count2;i++){
			cargosIdentifiers.push_back(manager.getNextId());
		}
	}
}
...
{
	this->count                 += cp->count2;
	this->cargo_days_in_transit += cp->days_in_transit * cp->count2;
}
...
/* static */ bool CargoList<Tinst, Tcont>::TryMerge(CargoPacket *icp, CargoPacket *cp)
{
	if (Tinst::AreMergable(icp, cp) &&
			icp->count2 + cp->count2 <= CargoPacket::MAX_COUNT) {
		icp->Merge(cp);
		return true;
	} else {
		return false;
...
void VehicleCargoList::AddToMeta(const CargoPacket *cp, MoveToAction action)
{
	this->AssertCountConsistency();
	this->AddToCache(cp);
	this->action_counts[action] += cp->count2;
	this->AssertCountConsistency();
}
...
void VehicleCargoList::AgeCargo()
{
	for (ConstIterator it(this->packets.begin()); it != this->packets.end(); it++) {
		...
		cp->days_in_transit++;
		this->cargo_days_in_transit += cp->count2;
	}
}
	...
	while (sum < this->count) {
		...
		switch (action) {
			...
			case MTA_TRANSFER:
				...
				share = payment->PayTransfer(cp, cp->count2);
 ...
 */
uint VehicleCargoList::Return(uint max_move, StationCargoList *dest, StationID next,Station *station,Vehicle *vehicle)
{
	max_move = min(this->action_counts[MTA_LOAD], max_move);
	this->PopCargo(CargoReturn(this, dest, max_move, next,station,vehicle));
	return max_move;
}
 ...
 */
uint VehicleCargoList::Shift(uint max_move, VehicleCargoList *dest,Vehicle *v)
{
	max_move = min(this->count, max_move);
	this->PopCargo(CargoShift(this, dest, max_move,v));
	return max_move;
}
 ...
 */
uint VehicleCargoList::Unload(uint max_move, StationCargoList *dest, CargoPayment *payment,Vehicle *vehicle,Station *station)
{
	uint moved = 0;
	if (this->action_counts[MTA_TRANSFER] > 0) {
		uint move = min(this->action_counts[MTA_TRANSFER], max_move);
		this->ShiftCargo(CargoTransfer(this, dest, move,vehicle,station));
		moved += move;
	}
	if (this->action_counts[MTA_TRANSFER] == 0 && this->action_counts[MTA_DELIVER] > 0 && moved < max_move) {
		uint move = min(this->action_counts[MTA_DELIVER], max_move - moved);
		this->ShiftCargo(CargoDelivery(this, move, payment,vehicle,station));
		moved += move;
	}
	return moved;
}
	...
	while (max_move > moved) {
		for (Iterator it(this->packets.begin()); it != this->packets.end();) {
			...
			if (prev_count > max_move && RandomRange(prev_count) < prev_count - max_move) {
				if (do_count && loop == 0) {
					(*cargo_per_source)[cp->source] += cp->count2;
				}
				++it;
				continue;
			}
			...
			if (cp->count2 > diff) {
				...
				if (loop > 0) {
					...
					if (do_count) (*cargo_per_source)[cp->source] += cp->count2;
					++it;
				}
			} else {
				it = this->packets.erase(it);
				...
				moved += cp->count2;
				this->RemoveFromCache(cp, cp->count2);
				delete cp;
			}
		}
		loop++;
 ...
 */
uint StationCargoList::Reserve(uint max_move, VehicleCargoList *dest, TileIndex load_place, StationIDStack next_station,Vehicle *vehicle,Station *station)
{
	return this->ShiftCargo(CargoReservation(this, dest, max_move, load_place,vehicle,station), next_station, true);
}
 ...
 */
uint StationCargoList::Load(uint max_move, VehicleCargoList *dest, TileIndex load_place, StationIDStack next_station,Vehicle *vehicle,Station *station)
{
	...
	if (move > 0) {
		...
		return this->ShiftCargo(CargoLoad(this, dest, max_move, load_place,vehicle,station), next_station, true);
	}
}
